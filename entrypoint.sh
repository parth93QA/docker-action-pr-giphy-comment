...
giphy_response=$(curl -s "https://api.giphy.com/v1/gifs/random?api_key=$GIPHY_API_KEY&tage=thank%20you&rating=g")
echo Giphy Response - $giphy_response

gif_url=$("$giphy_response" | jq --raw-output .data.images.downsized.url)
